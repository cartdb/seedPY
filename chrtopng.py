...
def decode_color_code(color):
    ...
    try:
        ...
        raise Exception("Error: invalid color code.")
    red = color >> 16
    green = (color >> 8) & 0xff
    blue = color & 0xff
...
def parse_arguments():
    ...
    try:
        ...
        raise Exception("Error: invalid option. See the readme file.")

    if len(args) != 2:
        raise Exception("Error: invalid number of arguments. See the readme file.")
    ...
    try:
        ...
        raise Exception("Error getting input file size.")
    (charRowCount, remainder) = divmod(size, CHARS_PER_ROW * BYTES_PER_CHAR)
    if charRowCount == 0 or remainder:
        raise Exception("Error: invalid input file size.")

    # target file
    target = args[1]
    if os.path.exists(target):
        raise Exception("Error: the output file already exists.")
    dir = os.path.dirname(target)
    if dir != "" and not os.path.isdir(dir):
        raise Exception("Error: the output directory does not exist.")
